...
Clicking the extension's toolbar icon opens a small popup with the current tab's URL, a dropdown of your saved profiles, and two buttons: **Download now** starts it in the background (the app doesn't jump in front of you), **Open in app** brings up the window with the URL filled in. It works on any `http(s)` page — yt-dlp supports around 1,800 sites plus a generic extractor for pages with a media file on them. Right-click a link, a video element, or the page for **Download with yt-dlp**, which sends the link's target rather than the page you're on — handy on a search-results or playlist page, and it skips the popup entirely. **Alt+Shift+Y** opens the popup (change it under `chrome://extensions/shortcuts`).
...
- **Filename template builder** (Advanced → Filename & Output): click a field to insert it, and an example underneath shows what the file will actually be called — rendered from the previewed video's own metadata when there is one
- Advanced tab covering format/merge/remux options, filenames (including a length cap for long titles), playlists, subtitles (language list, preferred format, and conversion to `.srt`), thumbnails/metadata (conversion to jpg, description/info-json/comments files), audio extraction, network (proxy, rate limit, concurrent fragments, throttling, sleeps, user-agent/referer/extra headers, browser impersonation), auth & cookies (browser profile and Firefox container, video password, 2FA, `.netrc`), SponsorBlock, geo-bypass, post-run commands, debug flags, per-extractor arguments, and a raw extra-arguments passthrough
- Preview a video and its actual subtitle languages appear as buttons under the card — click to add or remove them
- yt-dlp's own config files are ignored by default (`--ignore-config`), so what the Advanced tab says is what runs; Advanced → Debug turns that off if you keep options in one, and lists any it finds on this machine
...
- yt-dlp update check on launch (quietly skipped if offline) — the sidebar status turns yellow with an **Update to …** button when a newer release exists. Standalone-binary installs update in-app via `yt-dlp -U`, Homebrew via `brew upgrade yt-dlp`, pip installs via the script's own interpreter's `pip`; distro-package installs get the right package-manager command printed instead. Click the yt-dlp status line to re-check any time. Advanced → Debug switches the **update channel** to nightly or master — when a site breaks, the fix usually lands there days before the next stable release; each channel is checked against its own release feed, and only the standalone binary can switch (a Homebrew or pip install follows its packager). **Clear cache** next to it runs `yt-dlp --rm-cache-dir`, the standard first step for 403s and signature errors. yt-dlp's extractors break often as sites change, and "update yt-dlp" is almost always the fix, so this is worth keeping green.
- Live per-item progress, speed/ETA, and a separate streaming Log tab. When yt-dlp doesn't know the total size — a live stream, or a server that sends no length — the row shows the downloaded size, rate and elapsed time with a moving bar instead of a stuck percentage.
